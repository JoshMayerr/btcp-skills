---
name: integrate-add-on-acquisition
description: Integrate a closed private equity add-on acquisition into a portfolio company's existing .xlsx financial model and capitalization table. Use when an analyst needs to map acquired-company financials, reflect the closing sources and uses, debt, cash, fees, rollover equity, management or seller ownership, incentive-pool changes, and security issuances, reconcile post-close pro forma results and fully diluted ownership, and produce auditable updated workbook copies from final transaction documents.
---
